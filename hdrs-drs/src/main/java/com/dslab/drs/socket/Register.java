 ...
 * @author Weli
 */
public class Register extends ContainerResponseState {
  ...
  Register(SocketRun sr, SocketGlobalVariables global) {
    this.sr = sr;
    this.globals = global;
  }
  ...
  public void execute() {
    ...
    sr.increaseRegisterCount();
    globals.logContainerTime(containerID, System.currentTimeMillis());//以Container ID 註冊
...
//        globals.decreaseAMExpectedContainer();
        ...
        if (info == null) {
          ...
          if (globals.getRegCount() == 0) {
            long firstReqTime = globals.getFirstRequestTime();
            globals.getContainerMng().setRequestTime(System.currentTimeMillis() - firstReqTime);
            globals.increaseRegCount();
          }
